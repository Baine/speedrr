...
class ScheduleModule:
    ...
    def __init__(
        ...
        module_configs: list[ScheduleConfig],
    ...
    def get_reduction_value(self) -> tuple[float, float]:
        ...
        upload_values = "; ".join(
            f"{cfg.start}-{cfg.end}: {self._format(reduction[0])}"
            for cfg, reduction in self.reduction_value_dict.items()
        )
        download_values = "; ".join(
            f"{cfg.start}-{cfg.end}: {self._format(reduction[1])}"
            for cfg, reduction in self.reduction_value_dict.items()
        )
        logger.info(f"<schedule> Upload reduction values = {upload_values}")
        logger.info(f"<schedule> Download reduction values = {download_values}")
    ...
    def _format(value: float) -> str:
        return "unlimited" if value == float("inf") else str(value)
...
class ScheduleThread(threading.Thread):
    ...
    def __init__(self, config: ScheduleConfig, module: ScheduleModule) -> None:
        ...
        if isinstance(self._config.upload, str):
            if self._config.upload.lower() == "unlimited":
                ...
                self._upload_reduce_by = (
                    int(self._config.upload[:-1]) / 100 * self._module._config.max_upload
                )
        else:
        ...
        if isinstance(self._config.download, str):
            if self._config.download.lower() == "unlimited":
                ...
                self._download_reduce_by = (
                    int(self._config.download[:-1]) / 100 * self._module._config.max_download
                )
        else:
        ...
        for day_offset in range(8):  # Search up to 7 days ahead
            ...
            if candidate_date.weekday() in self._days_as_int:
                candidate_time = datetime.combine(
                    candidate_date, time(hour, minute), tzinfo=self.timezone
                )
    ...
    def set_reduction(self) -> None:
        ...
        self._module.reduction_value_dict[self._config] = (
            self._upload_reduce_by,
            self._download_reduce_by,
        )
        self._module._update_event.set()

    def remove_reduction(self) -> None:
    ...
    def run(self) -> None:
        while True:
            next_start_occurrence = self.calculate_next_occurrence(
                self._start_hour, self._start_minute
            )
            next_end_occurrence = self.calculate_next_occurrence(self._end_hour, self._end_minute)

            logger.debug(
                f"<ScheduleThread> Next start occurrence: {next_start_occurrence}, "
                f"Next end occurrence: {next_end_occurrence}"
            )
            ...
            if next_start_occurrence > next_end_occurrence:
                ...
                sleeping_time = (
                    next_end_occurrence - datetime.now(tz=self.timezone)
                ).total_seconds()
                ...
                sleeping_time = (
                    next_start_occurrence - datetime.now(tz=self.timezone)
                ).total_seconds()
                logger.debug(f"<ScheduleThread> start<end, Sleeping for {sleeping_time} seconds")
